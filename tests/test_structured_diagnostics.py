...
def test_build_session_diagnostic_report_no_followup_on_success() -> None:
    """exit_code=0 must not produce misleading failure diagnostics."""
    report = build_session_diagnostic_report(0)
    assert report["exit_code"] == 0
    codes = {item["code"] for item in report["diagnostics"]}
    assert "command_failed_likely_sandbox" not in codes
    assert "command_failed_application" not in codes
