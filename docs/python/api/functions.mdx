...
<ParamField path="enforcement_mode" type="str">
  Linux rollout mode. For this compatibility release, `"auto"` retains the
  Landlock-first behavior. `"seccomp"` layers a static network baseline under
  Landlock, denying UDP, raw/non-IP sockets, and `io_uring_setup()` for
  restricted policies. `"landlock"` explicitly selects Landlock only.
</ParamField>
...
### Linux Static Seccomp Baseline

During this compatibility release, opt into the stronger network baseline with
`enforcement_mode="seccomp"`. Plain `block_network()` then denies non-Unix
sockets on every supported Linux kernel. Policies with TCP port exceptions use
Landlock V4+ for the port allowlist while seccomp denies UDP and other socket
families. The baseline also denies `io_uring_setup()`, including for workloads
that use io_uring only for file I/O. A future release will make this the
`"auto"` behavior.
