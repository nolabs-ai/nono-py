...
| Property | Type | Description |
|----------|------|-------------|
| `executable_identity` | `dict \| None` | `{"resolved_path": str, "sha256": str}` of the launched binary |
| `audit_event_count` | `int` | Number of audit events recorded |
| `audit_integrity` | `dict \| None` | `{"hash_algorithm", "event_count", "chain_head", "merkle_root"}` |
| `audit_attestation` | `dict \| None` | `{"predicate_type", "key_id", "public_key", "bundle_filename"}` |
