...
| `started` | `str` | ISO 8601 start time |
| `ended` | `str \| None` | ISO 8601 end time (settable) |
| `command` | `list[str]` | Command that was executed |
| `tracked_paths` | `list[str]` | Tracked directories |
| `snapshot_count` | `int` | Number of snapshots (settable) |
| `exit_code` | `int \| None` | Child process exit code (settable) |
| `merkle_roots` | `list[ContentHash]` | Chain of Merkle roots from each snapshot |
| `network_events` | `list[dict]` | Network audit events (same schema as `ProxyHandle.drain_audit_events()`) |
