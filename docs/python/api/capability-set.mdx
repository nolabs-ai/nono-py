...
On Linux during the compatibility release, use
`sandboxed_exec(..., enforcement_mode="seccomp")` to add the static filter that
also denies UDP, raw/non-IP sockets, and `io_uring_setup()`. The default
`"auto"` mode remains Landlock-first for one release.
</Note>
...
```python
proxy_only(proxy: ProxyHandle) -> None
```
...
<ParamField path="proxy" type="ProxyHandle" required>
  A running proxy handle from `start_proxy()`.
</ParamField>

```python
from nono_py import CapabilitySet, ProxyConfig, start_proxy

caps = CapabilitySet()
proxy = start_proxy(ProxyConfig(allowed_hosts=["api.openai.com"]))
caps.proxy_only(proxy)  # Only proxy traffic allowed

# Pass proxy env vars to sandboxed child
