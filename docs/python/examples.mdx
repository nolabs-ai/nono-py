...
# If every sandbox runs as a dedicated Unix UID, you can also pass
# max_processes=N. Do not use it as a per-sandbox process budget on a normal
# shared user account; RLIMIT_NPROC counts all processes owned by that UID.
...
```python
from nono_py import ProxyConfig, start_proxy
...
caps = CapabilitySet()
policy.resolve_groups(["proxy_web_demo"], caps)
...
The same resolver also accepts the canonical `allow_domain` key used by the
main nono crate's profile format. Domains not listed in the allowlist are
blocked by omission.

`resolve_groups()` blocks direct network access for proxy-filtered policy
groups. Call `caps.proxy_only(proxy)` after `start_proxy()` to permit only the
