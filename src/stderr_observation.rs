...
//! # Security note
//!
//! All input here originates from an untrusted child process's stderr. Paths
//! extracted are used only for diagnostic suggestions, never for access-control
//! decisions. Length caps prevent unbounded memory growth from adversarial output.

use nono::AccessMode;
use std::path::{Path, PathBuf};

/// Maximum bytes of stderr to analyse. Output beyond this limit is ignored.
const MAX_STDERR_BYTES: usize = 65_536; // 64 KiB

/// Maximum number of unique path hints to collect.
const MAX_PATH_HINTS: usize = 50;

/// Maximum number of lines to wait for structured properties after a denial
/// code before giving up and resetting state.
const MAX_STRUCTURED_PENDING_LINES: usize = 5;

/// Path-level hint extracted from a command's own error output.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct ObservedPathHint {
...
///
/// Input is treated as untrusted: output is capped at [`MAX_STDERR_BYTES`] and
/// at most [`MAX_PATH_HINTS`] path hints are collected.
#[must_use]
pub fn analyze_error_output(
    ...
    // Cap analysis to avoid unbounded work on adversarial output.
    let error_output = if error_output.len() > MAX_STDERR_BYTES {
        &error_output[..MAX_STDERR_BYTES]
    } else {
        error_output
    };
    ...
    let mut pending_structured_lines = 0usize;
    ...
    for line in error_output.lines() {
        ...
        if looks_like_structured_access_denial_code(line) {
            ...
            pending_structured_lines = 0;
        }

        if pending_structured_access_denial {
            pending_structured_lines += 1;

            // Give up waiting for structured properties after too many lines —
            // avoids accumulating hints for all subsequent unrelated lines.
            if pending_structured_lines > MAX_STRUCTURED_PENDING_LINES {
                ...
                if let (Some(path), Some(access)) = (
                    ...
                    if observed.len() < MAX_PATH_HINTS {
                        observed
                            .entry(path)
                            .and_modify(|existing| {
                                *existing = merge_access_modes(*existing, access)
                            })
                            .or_insert(access);
                    }
                    pending_structured_access_denial = false;
...
fn extract_relative_write_path_from_line(line: &str, current_dir: &Path) -> Option<PathBuf> {
    ...
    // Reject absolute paths, option-like tokens, control chars, and directory
    // traversal sequences — all of which indicate the candidate is not a simple
    // relative path produced by the child's normal error output.
