...
pub(crate) fn attach_nono_error_diagnostics(py: Python<'_>, err: &PyErr, error: &NonoError) {
    ...
    // setattr failures are silently ignored: we're decorating an exception that
    // is about to be raised regardless, and some exception types disallow extra
    // attributes. The diagnostic fields are best-effort enrichment, not critical.
    let _ = value.setattr(
        "diagnostic_code",
...
pub(crate) fn build_session_report_from_exec(
    ...
    // Follow-up hints (e.g. command_failed_likely_sandbox) are only meaningful
...
/// Narrow a `ReadWrite` inferred access to just the missing half when the
/// capability already grants one direction. Returns `None` when the existing
/// capability already covers the inferred access.
fn narrow_missing_access(granted: AccessMode, inferred: AccessMode) -> AccessMode {
    match (granted, inferred) {
...
fn actionable_observed_access(
    ...
    Some(narrow_missing_access(cap.access, inferred))
}
...
fn remediation_for_observed_hint(
    ...
    if let Some(cap) = closest_covering_capability(caps, &hint.path) {
        return NonoRemediation::GrantPath {
            ...
            access: narrow_missing_access(cap.access, hint.access),
        };
    }
    // The path came from the child's own stderr (untrusted). The filesystem
    // probe below is a best-effort hint only — it cannot affect access control.
...
///
/// Uses empty denial/violation lists. Follow-up hints (e.g.
/// `command_failed_likely_sandbox`) are only appended when `exit_code != 0`.
#[pyfunction]
